...
void conout(UBYTE c);
#endif
/* Lê um caractere do console (bloqueante) */
extern UBYTE conin(void);

/* Imprime string no console (espera terminador '$' ou '\0') */
extern void prt_line(const char *str);

/* Entrada principal do BDOS: D0 = código da função (16-bit), D1 = parâmetro (32-bit) */
extern UWORD _bdos(UWORD func, ULONG param);

/* Retorna o ponteiro para o handler de exceção/trap em Assembly */
extern UBYTE *traphndl(void);

/* Altera os atributos de arquivo no FCB (R/O, System, etc) */
extern BOOLEAN set_attr(struct fcb *fcbp, UWORD attr);

/* Lê um setor físico/lógico da área de diretório do disco */
//extern UWORD dir_rd(struct fcb *fcbp, ULONG sector);
extern UWORD dir_rd(ULONG sector);

extern void initexc(void *excvec);

extern UWORD dirscan(BOOLEAN (*fn)(struct fcb *, UWORD), struct fcb *fcbp, UWORD option);
extern void conout(UBYTE c);
...
/* Initialize the File System */
{
    ...
    _bdos(13,0L);                  /* reset disk system function */
