...
#include "biosdef.h"            /* Declarations for BIOS entry points */
                 ...
                 /* Assembly language unsigned divide routine */
                                /* in bdosif.s.  It's used because Alcyon C  */
                                /* can't do / or % without an external */

extern UWORD udiv(ULONG dividend, UWORD divisor, UWORD *rem);
/************************
*  do_phio entry point  *
************************/

/* iop is a pointer to a i/o parameter block */
UWORD do_phio(struct iopb *iop){
    MLOCAL UBYTE last_dsk;      /* static variable to tell which disk
                                     was last used, to avoid disk selects */
    struct dph *hdrp;         /* pointer to disk parameter header   */
    struct dpb *dparmp;       /* pointer to disk parameter block    */
    UWORD   rtn;              /* return parameter                   */
