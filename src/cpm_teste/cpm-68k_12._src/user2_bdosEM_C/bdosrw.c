...
/* Leitura e escrita em disco (rwflg: 0 = read, 1 = write) */
//UWORD rdwrt(UWORD rwflg, struct fcb *fcbp);
UWORD rdwrt(UWORD lsec, UBYTE *dma, UWORD parm);
/* Aloca um bloco de espaço no disco */
//WORD getaloc(struct fcb *fcbp);
WORD getaloc(UWORD blk);
/* Inverte a ordem dos bytes (Assembly) */
UWORD swap(UWORD val);

/* Escaneia diretório executando a função de callback (openfile, create, etc.) */
UWORD dirscan(BOOLEAN (*fn)(struct fcb *, UWORD), struct fcb *fcbp, UWORD option);

/* Função de abertura passada ao dirscan */
BOOLEAN openfile(struct fcb *fcbp, UWORD dirindx);

/* Fecha o arquivo */
UWORD close_fi(struct fcb *fcbp);

/* Função de criação de arquivo passada ao dirscan */
BOOLEAN create(struct fcb *fcbp, UWORD dirindx);

/* Trata erro de arquivo somente-leitura (retorna void) */
void ro_err(struct fcb *fcbp, UWORD dirindx);
...
void setblk(fcbp, index, wrdfcb, block)
